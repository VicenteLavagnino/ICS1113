...
def generate_model(PARAMETERS):
    ...
    for a in A:
        for b in B:
            for p in P:
                model.addConstr(Ua_b_p[a, b, p] <= Ta_p_i[a, p, len(I) - 1] + gp.quicksum(Ha_p_i[a, p, i] for i in range(0, len(I)-1)), name=f"Restriccion11.1_{a}_{b}_{p}")
                model.addConstr(Ua_b_p[a, b, p] <= Wa_b[a, b], name=f"Restriccion11.2_{a}_{b}_{p}")
                model.addConstr(Wa_b[a, b] + Ta_p_i[a, p, len(I) - 1] <= Ua_b_p[a, b, p] + 1, name=f"Restriccion11.3_{a}_{b}_{p}")
                model.addConstr(Wa_b[a, b] + gp.quicksum(Ha_p_i[a, p, i] for i in range(0, len(I)-1)) <= Ua_b_p[a, b, p] + 1, name=f"Restriccion11.4_{a}_{b}_{p}")
