...
for i in range(encuestados):
    nueva_muestra.append([edad[i], contorno_cabeza[i], distancia_hombro_cabeza[i], contorno_brazo[i], ancho_cabeza[i]])
...
#------------------------- Funcion Objetivo ---------------------------
objetivo = quicksum((b + quicksum(w_i[j] * nueva_muestra[i][j] for j in range(5)) - gramos_relleno[i]) ** 2 for i in range(encuestados))
...
if model.status == GRB.OPTIMAL:
    ...
    columnas = ["edad", "contorno_cabeza", "distancia_hombro_cabeza", "contorno_brazo", "ancho_cabeza"]
